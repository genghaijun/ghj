...
lies in computing, for each destination $d$, the lowest distance from each neighbor $x$ to $d$ (i.e., $C_{x}(d)$), 
as well as the lowest distance from $x$ to the default next hop $b$ for $d$ (i.e., $C_{x}(b)$).
All the other costs, including $C_{c}(d), C_{b}(d)$ and $C_{x}(c)$, are already available 
...
Typical LFA implementations of nowaday router vendors naively maintain a SPT for each neighbor $x$, 
thus requires $O(k)$ time and space. For nodes with a large number of neighbors, this will become 
...
For example, DMPA adopts $C_{c}(u)-C_{c}(x)+L(u,d)<C_{c}(d)$, where $u$ is a descendent of $x$, 
and the left-hand equation is always no less than $C_{x}(d)$.
...
\begin{algorithm}[htb]
\KwIn{Network graph $G=(V,E)$ and $T_c$}
\KwOut{$N_c(d), (\forall d \in V \land d \neq c) $}
%\SetAlgoNoLine
%\SetAlgoNoEnd
\For{each neighbor $x$ of $c$}{
\For{$v \in V$}{
 %$v.visited \leftarrow floating$\;
 $C_{c}^{G_{(c,x)}}(v) \leftarrow C_c(v)$\;
...
%$c.visited=anchored$\;
 ...
 $newdist\leftarrow C_c(x)+\triangle$\;
 $\triangle\leftarrow L(c,x)-weight$\;

 ENQUEUE$(\{x,(c,newdist,\triangle)\})$\;
%\begin{shaded}
...
$\{m,(n,p,\triangle)\}\leftarrow$ EXTRACTMIN(Q)\;
$P_c^{G_{(c,x)}}(m)\leftarrow n$\;
%$m.visited\leftarrow anchored$\;
$C_c^{G_{(c,x)}}(m)\leftarrow p$\;

\For{$d\in D_c(m)$}
{
$C_{c}^{G_{(c,x)}}(d)\leftarrow C_c(d)+\triangle$\;
...
\For {each neighbor $q$ of $m$}{
\If {$C_c(q)>C_{c}^{G_{(c,x)}}(m)+L(m,q)$ }
{

$newdist \leftarrow C_{c}^{G_{(c,x)}}(m)+L(m,q)$\;
$\triangle\leftarrow newdist-C_c(q)$\;
ENQUEUE$(q,(m,newdist,\triangle))$\;
}
\Else
{
$C_{x}(q)\leftarrow C_{c}(q)+C_c(x)$\;
}
}
}
...
\Return $N_c(d), (\forall d \in V \land d \neq c)$
\caption{IAC}
\label{mnpe}
\end{algorithm}
\iffalse
...
According to the above discussions, algorithm IAC is proposed to compute all of the backup next-hop set which are satisfied the LFA rule. The inputs of the IAC are the network topology $G=(V,E)$ and $T_{c}$, and the output is the backup next-hop set from node $c$ to all  other nodes in the network.
IAC maintains a priority queue $Q$, which contains a subset of nodes together with three attributes. In particular, each element in $Q$ is
of the form ${(u,(v,q,\triangle))}$, where $v$ is the potential parent
for node $u$, $q$ denotes a potential cost for node $u$ , and $\triangle$ is the potential cost change for node $u$ .
The IAC requires several iterations. In each iteration,
at the beginning,  both of the parameters $C_{c}^{G_{(c,x)}}(v), v\in V$  and  $P_{c}^{G_{(c,x)}}(v)$ are initialized
(lines 2 $\sim$ 4). We change the weight of the link  $(c,x)$  to $-L(c,x)$, and compute the cost change for $(c,x)$ (lines 5 $\sim$ 8), which is stored in variable $\triangle$.
The node $x$ id enqueued into the $Q$ by the ENQUEUE operation with its potential new parent $c$ , its potential new cost $newdist$, and the cost change $\triangle$ (line 9).
A node $m$ with the lowest potential cost change will be popped out from the priority queue $Q$ by the EXTRACTMIN operation, where node ID is  used as tie breaking.
It is then added to the tree, and used as the current node in the iteration, while its attributes like lowest cost and parent  are also set to a permanent value in the $G_{(c,x)}$ (lines $12 \sim 13$).
For all the nodes $d \in D_c(m)$, their cost is updated accordingly (line 15), and the corresponding shortest cost $C_x(d)$ is then computed (lines $16 \sim 19$).
For each neighbor $q$ of $m$, we compute the  potential cost of $q$. The node $q$ will be inserted into the $Q$ if its new potential cost is smaller than the old value (lines 21 $\sim$ 24).
While compute the shortest cost $C_x(q)$ when the potential cost of $q$ is smaller than the old value (lines 26).
Then, we will restore the weight of the link $(c,x)$ (line 27).
At last lines 28-31 are using to compute all the LFA next hop set for node $c$.

%\subsection{Example}
In this section, we will employ a simple example to explain how ICA works. Fig. \ref{spttree21} shows a network topology consisting of 5 nodes and 6 edges. Fig. \ref{spttreechange22} shows the shortest path tree $T_c$ rooted at the node $c$.
Here we will describe how to compute $T_c^{G(c,a)}$ based on $T_c$.
First, the root node $c$ is marked as $anchored$, all of the nodes except $c$ are marked as $floating$.
For all the $floating$ nodes,
we check whether they have direst link to the anchored nodes, and calculate the new cost and the change in cost, which gives
queue \{$(a,(c,-3,-6))$\}.
In the first iteration, $(a,(c,-3,-6))$ will be selected and  $a$ will be marked as $anchored$. The original subtree rooted at $d$  is considered together. Therefore the node $d$ is selected and marked as $anchored$ at the same time. Since $d$ has an outing edge to $e$, the new cost and $\delta$ of $e$ is calculated, which gives queue \{$(e,(e,3,-5))$\}. Node $d$ is marked as anchored in the next iteration.
The new shortest path tree is given in the
Fig. \ref{spttreechange23}.
We have $d\in D_c(a)$ from Fig. \ref{spttreechange22} , so $C_a(d)=C_c(d)-C_c(a)=3$.
Because $e \notin D_c(a)$ and $e \in D_c^{G_{(c,a)}}(a)$, we have $C_a(e)=C_c^{G_{(c,a)}}(e)+C_c(a)=6$ from Theorem \ref{computeneighborcost}.
Since $b \notin D_c(a)$ and $b \notin D_c^{G_{(c,a)}}(a)$, we have $C_a(b)=C_a(c)+C_c(b)=8$.
...
\end{theorem}
\begin{proof}
Since only the weight of the link $(c,x)$ is changed to $-L(c,x)$,
assume that there is a negative cycle $P=(c,x,...c)$, the cost
of the path is smaller than 0.
Therefore, we have $C_c(x)+Cost(x,c)<0$, where $Cost(x,c)$ denote the cost from $x$ to $c$ in the path $P$.
Since $C_c(x)=-L(c,x)$, we can get
$Cost(x,c)<L(c,x)$, that is to say
there exists a path from $x$ to $c$ whose cost is smaller than $L(c,x)$,
this is impossible. We can conclude that there is no negative cycle in the $G'$.
Therefore, there must exist a shortest path tree in the $G'$, the Theorem is correct according to \cite{Narv2002New}.
\iffalse
...
Given a shortest path tree $T_c$ and a network topology  $G'=G_{(c,x)}, (c,x)\in T_c$, for any node $d \notin D_c(x)(d\neq c \land d\neq x)$
\begin{itemize}
\item[(1)] If $d\in D_c^{G'}(x)$, then we have  $C_x(d)<C_x(c)+C_c(d)$.
 \item[(2)] If $C_x(d)<C_x(c)+C_c(d)$ is satisfied,  then we have $d\in D_c^{G'}(x)$.
\end{itemize}
\end{theorem}
\begin{proof}
(1)Since $d \in D_c(x)$ and $d\in D_c^{G'}(x)$, the node $d$ or its ancestor(s) must be enqueued into the $Q$ using ENQUEUE operation.
Assume that the cost difference for node $d$ is  $\triangle$, the potential cost of node $d$ is $C_c(d)+\triangle$.
In $T_c^{G'}$, we have $C_c^{G'}(d)=C_c^{G'}(x)+C_x^{G'}(d)$.
Since $C_c^{G'}(x)=-L(c,x)$, we have $C_c^{G'}(d)=C_x^{G'}(d)-L(c,x)$.
Because the shortest path from node $x$ to $d$ is not going through node $c$ in both of the two networks $G'$ and $G$, we have $C_x^{G'}(d)=C_x(d)$.
Because $C_c^{G'}(d)=C_c(d)-\delta$,
we have $C_x(d)-L(c,x)=C_c(d)-\triangle$.
Therefore, we have $C_x(d)<L(c,x)+C_c(d)$.
Since $C_c(x)=L(c,x)$, we have $C_x(d)<C_c(x)+C_c(d)$.

(2)
Since $C_x(d)<C_x(c)+C_c(d)$, the shortest path from node $c$ to node $d$ is not via node $c$.
When the weight of edge $(c,x)$ is changed from $L(c,x)$ to $-L(c,x)$, there exists a path $P=c,x,...,d$ from $c$ to $d$, and the cost of which is $C_c(x)+C_x(d)$.
Because $C_c(x)=-L(c,x)$, the cost of path $P$ is $C_x(d)-L(c,x)$.
Therefore, the potential cost change for node $d$ is $C_c(d)-C_x(d)+L(c,x)>0$.
The node $d$ will be enqueued into the $Q$ using ENQUEUE operation.
...
Given a shortest path tree $T_c$ and a network topology  $G'=G_{(c,x)}, (c,x)\in T_c$, for any node $d \notin D_c(x)(d\neq c \land d\neq x)$
if  $d\notin D_c^{G'}(x)$, then we have  $C_x(d)=C_x(c)+C_c(d)$.
\end{lemma}
\begin{proof}
From Theorem \ref{newsptposition}, we have $C_x(d)\geq C_x(c)+C_c(d)$ when
both of the conditions $d \in D_c(x)$ and $d\notin D_c^{G'}(x)$ are established simultaneously.
...
From Theorem \ref{newsptposition}, we can get $C_{x}^{G'}(d)=C_{x}(d)$.
...
(1) If $d \in D_c(x)$, the lines 16-17 are executed.

(2)If $d \notin D_c(x) \land d \notin D_c^{G'}$, the lines 18-19 are executed.

(3)When the node $d \notin D_c(x)$ and is not enqueued into the $Q$, 
the node $d$  must not be a descendant of $x$ in $T_c^{G'}$.
The value of $C_x(d)$ can be computed by line 26.

According to Theorem \ref{computeneighborcost}, all of the $C_x(d), d \in V$ will be calculated when the lines ($1 \sim 27$) of the IAC are executed.
\end{proof}
...
\end{theorem}
\begin{proof}
From Theorem \ref{algorithmcorrect}, all of the $C_x(d), d \in V$ will be calculated when the lines ($1 \sim 27$) of the IAC are executed.
As long as we know the above values, we can use the LFA rule to compute all the LFA next hop set for node $c$ (lines $29 \sim 31$). 
\end{proof}
