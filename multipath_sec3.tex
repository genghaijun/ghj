...
we check whether they have direst link to the anchored nodes, and calculate the new distance and the change in distance, which gives
queue \{$(a,(c,-3,-6))$\}.
In the first iteration, $(a,(c,-3,-6))$ will be selected and  $a$ will be marked as $anchored$. The original subtree rooted at $d$  is considered together. Therefore the node $d$ is selected and marked as $anchored$ at the same time. Since $d$ has an outing edge to $e$, the new distance and $\delta$ of $e$ is calculated, which gives queue \{$(e,(e,3,-5))$\}. Node $d$ is marked as anchored in the next iteration.
