...
$d$ becomes $x$'s descendant in $T_{c}^{G_{(c,x)}}$, we have $Length(P)=C_x(d)$, where $P=(x,y,...,d)$ is the  path from $x$ to $d$ in the $T_{c}^{G_{(c,x)}}$.
\end{theorem}
\begin{proof}
We prove by contradiction. 
Since $d\in D_c^{G_{(c,x)}}(x)$, we have $(c,x)\notin P$.

(1)Assume that $Length(P)<C_x(d)$,  that is to say the path $P$ has a lower cost than $C_x(d)$. This is impossible because $C_x(d)$ has the lowest cost from $x$ to $d$.

(2)Since $d\in D_c^{G'}(x)$, we can get $C_c^{G(c,x)}(d)=C_c^{G(c,x)}(x)+Length(P)$.
If $Length(P)>C_x(d)$, we have $C_c^{G(c,x)}(d)>C_c^{G(c,x)}(x)+C_x(d)$.
This is impossible according to the Theorem \ref{newspt}.

\end{proof}
\begin{theorem}\label{newsptposition}
For $c$'s neighbor $x$ and any node $d$ (other than $c$ and $x$) that is not $x$'s descendant in $T_c$,
$d$ becomes $x$'s descendant in $T_{c}^{G_{(c,x)}}$ if and only if $C_x(d)<C_x(c)+C_c(d)$.
\end{theorem}
\begin{proof}
...
and later positioned in the subtree rooted at $x$ and becomes $x$'s descendant. From Theorem \ref{newspt},
