...
%$c.visited=anchored$\;
 ...
 ENQUEUE$(\{x,(c,C_c(x)-2*L(c,x),\delta)\})$\;
%\begin{shaded}
...
$\{m,(n,p,\delta)\}\leftarrow$ EXTRACTMIN(Q)\;
%$P_c^{G_{(c,x)}}(m)\leftarrow n$\;
%$m.visited\leftarrow anchored$\;
$C(m)\leftarrow p$\;
the descendants of node $m$ is append below node $m$\;
...
\If {$d \notin D_c(x)$ and $d$ is now a descendant of $x$}
{$C_{x}(d)\leftarrow C_{c}^{G_{(c,x)}}(d)+C_{c}(x)$\;
}
\If {$d \notin D_c(x)$ and $d$ is not now a descendant of $x$}
{$C_{x}(d)\leftarrow C_{c}(d)+C_c(x)$\;
}
\If {LFA is satisfied}
 {
...
of the form ${(u,(v,q,\delta))}$, where $v$ is the potential parent
for node $u$, $q$ denotes a potential cost for node $u$ , and $\delta$ is the potential cost change for node $u$.
...
(lines 2 $\sim$ 4). We change the weight of the link  $(c,x)$  to $-L(c,x)$, and compute the potential cost change for $(c,x)$ (lines 5 $\sim$ 8), which is stored in variable $\delta$.
The node $x$ is enqueued into the $Q$ by the ENQUEUE operation with its potential  parent $c$, its potential cost $newdist$, and the potential cost change $\delta$ (line 9).
...
we have $C_x(d)-L(c,x)=C_c(d)-\delta$.
