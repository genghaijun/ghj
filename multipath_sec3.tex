...
For $c$'s neighbor $x$ and any node $d$ (other than $c$ and $x$) that is not $x$'s descendent in $T_c$, 
$d$ becomes $x$'s descendent in $T_{c}^{G_{(c,x)}}$ if and only if $C_x(d)<C_x(c)+C_c(d)$.
\end{theorem}
\begin{proof}
...
which means $C_{c}(d)>-L(c,x)+C_{x}(d)$. So at some time, the condition of line 13 in Algorithm 1
will be triggered, and $d$ will be inserted into $Q$ by the ENQUEUE operation in line 16, 
and later positioned in the subtree rooted at $x$ and becomes $x$'s descendent. From Theorem \ref{newspt}, 
we know the subtree is part of $T_{c}^{G_{(c,x)}}$.

We then prove if $d\in D_c^{G'}(x)$, then  $C_x(d)<C_x(c)+C_c(d)$. Since $d \in D_c(x)$ and $d\in D_c^{G'}(x)$, the node $d$ or its ancestor(s) must be enqueued into the $Q$ using ENQUEUE operation.
