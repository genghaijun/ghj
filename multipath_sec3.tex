...
critical to IAC's correctness and efficiency.
...
\end{theorem}
\begin{proof}
To compute all the backup next-hop set from node $c$ to other nodes in the network. The IAC need to run the iSPF algorithm $k$ times, where $k$ is the number of neighbors of node $c$. Let $N_{i}$  be the number of
potential affected nodes, and $M_{i}$ be the number of edges
attached to one of these nodes. The computational complexity in IAC for this neighbor $i$ is $O(M_{i}lgN_{i})$ according to \cite{Narv2002New}.
