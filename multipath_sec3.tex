...
For each neighbor $x$ of $c$, the dual graph $G_{(c,x)}$ contains no negative cycle, and Algorithm 1 constructs the corresponding SPT $T_{c}^{G_{(c,x)}}$.
\end{theorem}
\begin{proof}
%the weight of the link $(c,x)$ is changed to $-L(c,x)$,
We prove by contradiction. Assume there exists a negative cycle in $G_{(c,x)}$, then it must go through $(c,x)$ since 
...
$(c,x)$ and $(x,...,c)$ in this cycle be $-L(c,x)$ and $Length(x,y,...,c)$ respectively, 
then $-L(c,x)+Length(x,y,...,c)<0$. That means $Length(x,y,...,c)<L(c,x)$, which contradicts with our triangle inequality 
assumption that $L(c,x)$ is the lowest cost between $c$ and $x$. 
Therefore, there must exist a SPT in $G'$, and the correctness of tree construction can be proven by following 
the proof of the ball-and-string algorithm \cite{Narv2002New}.
