...
as well as the lowest  cost from $x$ to the default next hop $b$ for $d$ (i.e., $C_{x}(b)$).
...
(lines 2 $\sim$ 4). We change the weight of the link  $(c,x)$  to $-L(c,x)$, and compute the potential cost change for $(c,x)$ (lines 5 $\sim$ 8), which is stored in variable $\triangle$.
The node $x$ is enqueued into the $Q$ by the ENQUEUE operation with its potential  parent $c$, its potential cost $newdist$, and the potential cost change $\triangle$ (line 9).
...
For all the descendants of node $m$ in the old shortest path tree $T_c$, their lowest cost is updated together (line 15).
For each neighbor $q$ of $m$, we compute the  potential cost of $q$. The node $q$ will be inserted into the $Q$ if its  potential cost is smaller than the old value (lines 17 $\sim$ 20).
The weight of the link $(c,x)$ is restored when the $Q$ is empty.
For each neighbor $x$ of $c$, the value of $C_x(d), d\in V$ is computed and the  backup next hop is added according to corresponding LFA (lines 22 $\sim$ 31).
\iffalse
...
Assume that the potential cost change for node $d$ is  $\triangle$, the potential cost of node $d$ is $C_c(d)+\triangle$.
