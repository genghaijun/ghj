...
\begin{algorithm}[t]\label{IACalg}
...
for a give node, the lowest cost from $c$ to it in graph $G$, and its descents 
...
i.e., cost of link $(c,x)$ is changed from $L(c,x)$ to $-L(c,x)$ (line 4 $\sim$ 5). 
...
Then we extract the node $m$ with the lowest cost in $Q$ and fix its position 
as well as its descendents' positions, in the new tree (line 7 $\sim$ 8), 
...
Note that $m$'s descendents will not be reinserted into $Q$ 
...
Later, we will prove that this incremental tree construction actually constructs the shortest path 
tree for $G_{(c,x)}$, where $G_{(c,x)}$ is the dual graph of $G$ with the sign of $(c,x)$'s cost reversed, 
and $C_{x}(d)$ is correctly computed. 
But before the formal proof, we use the following example to explain how ICA works. 
...
For each neighbor or $c$, i.e., $a$ (or $b$), we use Algorithm 1 to compute a shortest path tree 
...
\fi
For the neighbor $a$, according to line 19 $\sim$ 24 in Algorithm 1, 
