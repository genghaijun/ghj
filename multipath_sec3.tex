...
For each neighbor $x$ of $c$, the dual graph $G_{(c,x)}$ contains no negative cycle, and Algorithm \ref{IAC-alg} constructs the corresponding SPT $T_{c}^{G_{(c,x)}}$.
\end{theorem}
\begin{proof}
%the weight of the link $(c,x)$ is changed to $-L(c,x)$,
We prove by contradiction. Assume there exists a negative cycle in $G_{(c,x)}$, then the cycle must go through $(c,x)$ since
...
$d$ becomes $x$'s descendant in $T_{c}^{G_{(c,x)}}$, we have 
$C_c^{G(c,x)}(d)=C_c^{G(c,x)}(x)+C_x(d)$.

\end{theorem}
\begin{proof}
We prove by contradiction.
Let $P=(x,y,...,d)$ be a path from $x$ to $d$ in the $T_{c}^{G_{(c,x)}}$, and $Length(P)$ be the cost of path $P$.
...
There are two cases:
(A)If node $c$ is on the shortest path from $x$ to $d$, we can get
$C_x(d)=C_x(c)+C_c(d)$.
Assume that $Length(P)>C_x(d)$, because $C_c^{G(c,x)}(d)>C_c^{G(c,x)}(x)+C_x(d)$, we have $C_c^{G(c,x)}(d)>C_c^{G(c,x)}(x)+C_x(c)+C_c(d)$.
Since $C_c^{G(c,x)}(x)+C_x(c)=0$, we can get $C_c^{G(c,x)}(d)>C_c(d)$.
This is impossible.
(B)If node $c$ is not on the shortest path from $x$ to $d$.
If $Length(P)>C_x(d)$, we have $C_c^{G(c,x)}(d)>C_c^{G(c,x)}(x)+C_x(d)$.
This is impossible according to the Theorem \ref{newspt}.

Therefore, we can get $Length(P)=C_x(d)$.
Because $d\in D_{c}^{G_{(c,x)}}(x)$, we have $C_c^{G(c,x)}(d)=C_c^{G(c,x)}(x)+Length(P)$.
Therefore, we have $C_c^{G(c,x)}(d)=C_c^{G(c,x)}(x)+C_x(d)$.
\end{proof}
...
We then prove if $d\in D_c^{G(c,x)}(x)$, then  $C_x(d)<C_x(c)+C_c(d)$. Since $d \notin D_c(x)$ and $d\in D_c^{G(c,x)}(x)$, the node $d$ or its ancestor(s) must be enqueued into the $Q$ using ENQUEUE operation.
...
Since $C_c^{G(c,x)}(x)=-L(c,x)$, we have $C_c^{G(c,x)}(d)=C_x(d)-L(c,x)$.
Because $C_c^{G(c,x)}(d)=C_c(d)-\delta$,
