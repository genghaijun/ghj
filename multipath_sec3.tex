...
%For all the nodes in the subtree of node $m$ i is considered together
For all the descendants of node $m$ in the old shortest path tree $T_c$,, their cost is updated together (line 15).
For each neighbor $q$ of $m$, we compute the  potential cost of $q$. The node $q$ will be inserted into the $Q$ if its new potential cost is smaller than the old value (lines 17 $\sim$ 20).
Then, the weight of the link $(c,x)$ is restored.
For each neighbor $x$ of $c$, the value of $C_x(d), d\in V$ is computed and the corresponding backup next hop is added according to LFA (lines 22 $\sim$ 31).
\iffalse
...
In the first iteration, $(a,(c,-3,-6))$ will be selected. The original subtree rooted at $a$  is considered together. Therefore the node $d$ is selected at the same time. 
Since $d$ has an outing edge to $e$, the new cost and cost difference of $e$ is calculated, which gives queue \{$(e,(e,3,-5))$\}. Node $d$ is selected in the next iteration.
The new shortest path tree is given in the
Fig. \ref{spttreechange23}.
Because $d\in D_c(a)$, we have $C_a(d)=C_c(d)-C_c(a)=3$.
Because $e \notin D_c(a)$ and $e \in D_c^{G_{(c,a)}}(a)$, we have $C_a(e)=C_c^{G_{(c,a)}}(e)+C_c(a)=6$.
Since $b \notin D_c(a)$ and $b \notin D_c^{G_{(c,a)}}(a)$, we can get $C_a(b)=C_a(c)+C_c(b)=8$.
