...
according to the criteria of \textbf{LFC}, or \textbf{NPC}, or \textbf{DC}, for different scenarios. 
By carefully investigating each criterion in Section \ref{criteria}, we can see that the difficulty 
...
All other costs, including $C_{c}(d), C_{b}(d)$ and $C_{x}(c)$, are already available since $c$ maintains
a shortest path tree rooted at itself.

The computation is actually embedded in a dynamic shortest path tree (SPT) algorithm, which takes 
a graph and the corresponding SPT as input, and performs incremental shortest path computation 
with the sign of some specific link cost simply reversed, i.e., from $x$ to $-x$. 
...
For all the nodes $d \in D_c(m)$, their cost is updated accordingly (line 15), and the corresponding cost $C_x(d)$ is then computed (lines $16 \sim 21$).
For each neighbor $q$ of $m$, we compute the  potential cost of $q$. The node $q$ will be inserted into the $Q$ if its new potential cost is smaller than the old value (lines 22 $\sim$ 26).
...
\end{theorem}
\begin{proof}
For any neighboring node $x$ of $c$, the $C_x(d), d \in V$ will be calculated when the lines (1-27) of the IAC are executed. As long as we know the above values, we can use the LFA rule to compute all the LFA next hop set for node $c$. Therefore, the Theorem is established.
\end{proof}
