...
packets destined to node $d$ ($c \ne d$), where $c$ detects that the default next hop for $d$ cannot be used,
and has to find an alternate next hop among $c$'s neighboring nodes.
...
\includegraphics[width=1.5in]{lfaexample}
...
\textbf{LFC:}
node $c$ can choose any neighboring node $x$ of $c$ satisfying $C_x(d)<C_x(c)+C_{c}(d)$,
which means when packets are routed from $x$ to $d$, they will not be routed back to $c$,
since $C_{x}(c)+C_{c}(d)$ is the lowest cost of any path from $x$ to $d$ that passing $c$.


\textbf{NPC:}
For packets destined to a destination $v$, node $c$ ($c \ne d$)
can forward them to its any neighboring node $x$ as long as $C_x(d)<C_x(f)+C_f(d)$, where $f$ is the default next hop from $c$ to $d$,
and there will be no forwarding loop in the induced forwarding path.

\textbf{DC:}
For packets destined to a destination $d$, node $c$ ($c \ne d$)
can forward them to its any neighboring node $x$ as long as $C_x(d)<C_c(d)$,
and there will be no forwarding loop in the induced forwarding path.


The LFC rule can be interpreted as the node $s$ is not on the shortest path from $x$ to $d$, NPC shows that the node $f$ is not on the shortest path from $x$ to $d$. DC rule indicates that the neighbor $x$ is closer to the destination $d$ then the node $s$ itself.
...
the node $c$ can compute a candidate set of next-hops $N_{c}(d)$
for each destination $d$ ($d\!\neq\! c$) employing LFA, so that when a packet destined to $d$ arrives at $c$,
$c$ can select a next-hop from $N_{c}(d)$ and forward this packet to.
