...
We use $C_c(v)$ to denote the lowest cost from node $c$ to node $v$ in the network, 
and use $N_{c}(v)$ to denote the set of next hops that $c$ can forward to when receiving a packet destined to the node $v$.
