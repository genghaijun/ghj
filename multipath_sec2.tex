...
For a neighboring node $u$ of $v$, i.e., $u\in N_{c}(v)$, it is clear that $C_u^G(v)=C_v^G(u)=L(u,v)$.
...
Then it builds a shortest path tree (SPT) $T_c^G$, where $c$ is the root of the tree, and all other nodes are included,
...
We use $D_c^G(v)$ to denote the descendants of node $v$ (itself is included) in $T_c$.
