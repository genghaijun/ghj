...
For the cost between two neighboring nodes $u$ of $v$, we further assume the triangle inequality  that $C_u^G(v)=C_v^G(u)=L^{G}(u,v)$, which is typically followed by ISPs in practice.
