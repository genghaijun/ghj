...
Although routing protocols automatically heal the network by computing new paths and reroute, they typically exhibit 
a slow response of hundreds of milliseconds or even longer.  
To support critical applications or guarantee a high quality of service, Internet Service Providers (ISPs) 
generally deploy a technique called Loop Free Alternates (LFA) for fast reroute. 
With LFA, alternative next hops are stored alongside with 
the default next hop in a router's forwarding table, and can be immediately activated in face of link or node failures. 
In this paper, we propose IAC-NA, an algorithm for computing these alternative next hops efficiently.
